...
התא הראשון של Part 3 בודק אם המשתנים ב-globals קיימים. אם כן — לא נעשה דבר. אם לא, קוראים את קבצי ה-pkl ו-`globals().update(...)` מזריק אותם למחברת כאילו Parts 1 ו-2 באמת רצו. בסביבת Colab, אם קבצי ה-pkl חסרים, נפתח דיאלוג `google.colab.files.upload()` שמאפשר למשתמש להעלות אותם.
